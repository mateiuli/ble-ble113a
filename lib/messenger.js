...
function Messenger(serialport) {
  this.bgLib = new bglib();

  // Dont need extra packet length header.
  this.bgLib.setPacketMode(false);

  // Initialize serialport
  this.serialport = serialport;

  // Start listening for serialport data
  this.serialport.on('data', this.parseIncomingPackets.bind(this));

  // Array of packets awaiting a response
  this.awaitingResponse = [];

  // Amount of time to wait before a packet timeout
  this.commandPacketTimeoutDuration = 10000; //60000

  /* After dongle reset, reopen port. */
  this.on('systemResetDone', this.onSystemResetDone.bind(this));

  /* How much time to wait after syste reset. */
  this.systemResetTimeout = 3000;
  this.systemReset();
}

util.inherits(Messenger, events.EventEmitter);

Messenger.prototype.onSystemResetDone = function() {
  var self = this;
  this.serialport.removeAllListeners('close');
  /* Open serial port. */
  this.serialport.open(function(err) {
    /**
     * Issue a systemReset command.
     * After the reset, serial port will get closed.
     */
     self.emit('ready');
  });
}

Messenger.prototype.systemReset = function() {
  /**
   * Send system reset to bring it in a known state.
   * Boot into main program
   */
  var self = this;

  /* After system reset, port is closed. */
  this.serialport.on('close', function() {
    /* Wait few seconds, after that emit dongle reset. */
    setTimeout(function() {
     self.emit('systemResetDone');
    }, self.systemResetTimeout);
  });
  ...
  this.serialport.open(function(err) {
    ...
    * After the reset, serial port will get closed.
    */
    self.execute(bglib.api.systemReset, [0]);
  });
}
...
function toHexString(byteArray) {
  ...
  for(var i = 0; i < byteArray.length; i++) {
    ...
    str += btoh[byte & 0xF0] + "" + btoh[byte & 0x0F] + " ";
  }

  return str;
  // return byteArray.map(function(byte) {
    // return "0x" + btoh[byte & 0xF0] + "" + btoh[byte & 0x0F];
...
Messenger.prototype.execute = function(command, params, callback) {
  if(!this.serialport.isOpen()) {
    callback && callback(new Error("Port not open."), null);
  }
...
Messenger.prototype.sendBytes = function(packet, bArray, callback) {
  // Send it along
  console.log("\nTX: cmd_class = " + packet.cClass + ", cmd_id = " + packet.cID);
  console.log(bArray);
  this.serialport.write(bArray);
...
Messenger.prototype.parseIncomingPackets = function(data) {
  ...
  var incomingPackets = this.bgLib.parseIncoming(data, function callbackSeeker(err, parsedPackets) {
    ...
    for (var i = 0; i < parsedPackets_length; i++) {
      ...
      console.log("RX: cmd_class = " + cClass + ", cmd_id = " + cID);
      console.log(data);
